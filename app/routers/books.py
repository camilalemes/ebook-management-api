...
class BookFormat(BaseModel):
    """Book format with file size information."""
    format: str
...
async def list_books(
    ...
    try:
        ...
        for book_data in books_data:
            # Convert format data to BookFormat objects
            book_formats = []
            for format_data in book_data['formats']:
                book_formats.append(BookFormat(
                    format=format_data['format'],
                    file_size=format_data['file_size'],
                    filename=format_data['filename']
                ))
...
@router.get("/libraries/{library_id}/books/{book_id}/download")
async def download_book(
    library_id: str, 
    book_id: int,
    format: Optional[str] = Query(None, description="Book format to download")
):
    """Download a book file from the specified library."""
    library_paths = get_library_paths()
    ...
    try:
        ...
        formats_list = book_data['formats']
        if not formats_list:
            raise HTTPException(status_code=404, detail=f"No files available for book {book_id}")
        
        # Convert format objects to lowercase format names for matching
        if isinstance(formats_list[0], dict):
            available_formats = [fmt['format'].lower() for fmt in formats_list]
        else:
            available_formats = [fmt.lower() for fmt in formats_list]
        
        # Determine which format to download
        target_format_name = None
        if format:
            # Look for specific format
            format_lower = format.lower()
            if format_lower in available_formats:
                target_format_name = format_lower
            else:
                raise HTTPException(
                    status_code=404, 
            ...
            if 'epub' in available_formats:
                target_format_name = 'epub'
            else:
                target_format_name = available_formats[0]
        
        # Build file path based on Calibre library structure
        # Calibre stores books in "Author/Title (ID)" folders
        book_folder = os.path.join(library_path, book_data['path'])
        
        if not os.path.exists(book_folder):
            raise HTTPException(status_code=404, detail=f"Book folder not found: {book_folder}")
        
        # Look for the file in the book folder
        target_extension = target_format_name
        
        # Try different possible filenames
        possible_files = [
            f"{book_data['title']}.{target_extension}",
        ]
        
        file_path = None
        for filename in possible_files:
            candidate_path = os.path.join(book_folder, filename)
            if os.path.exists(candidate_path):
                file_path = candidate_path
                break
        
        # If not found, scan the directory for any file with the target extension
        if not file_path:
            try:
                for file in os.listdir(book_folder):
                    if file.lower().endswith(f'.{target_extension}'):
        ...
        if not file_path or not os.path.exists(file_path):
            raise HTTPException(status_code=404, detail=f"Book file not found for format {target_format_name}")
